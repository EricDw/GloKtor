...
import net.publicmethod.domain.queries.QUERY_VALUE_FILE_NAME
...
class GloApiAttachmentsTests : GloApiTest
{
    ...
    @KtorExperimentalAPI
    @Test
    fun `given PAT when getAttachmentsForCard then return correct Attachments`() =
        runBlocking {
    ...
    @KtorExperimentalAPI
    @Test
    fun `given PAT when queryAttachmentsForCard then return correct Attachments`() =
        runBlocking {
            ...
            // Act
            val actual = gloApi.queryAttachmentsForCard(TEST_BOARD_ID_1, TEST_CARD_ID_1) {
                addFileName()
            }
